...
import { ALL_COLLIDABLE, pointInAABB } from './mapData';

const MAX_VISIBLE = 60;
// Sub-step size: at most 0.25 units per check — prevents tunnelling through thin cover
  ...
  useFrame((_, delta) => {
    ...
    if (!sharedState.matchRunning) {
      bulletMeshes.current.forEach(({ mesh, trail }) => {
        mesh.visible = trail.visible = false;
      });
      sharedState.bullets = [];
      return;
    }
    ...
    for (const bullet of sharedState.bullets) {
      ...
      // Sub-step movement so bullets can't tunnel through thin walls
      const totalDist = BULLET_SPEED * delta;
      const steps     = Math.max(1, Math.ceil(totalDist / MAX_SUBSTEP));
      const stepDist  = totalDist / steps;

      let stopped = false;
      for (let s = 0; s < steps && !stopped; s++) {
        bullet.position.addScaledVector(bullet.direction, stepDist);
        ...
        // ── Map boundary ──────────────────────────────────────────
        if (Math.abs(bx) > MAP_HALF || Math.abs(bz) > MAP_HALF) {
          bullet.alive = false;
          stopped = true;
          break;
        }

        // ── Wall / cover collision ────────────────────────────────
        for (const aabb of ALL_COLLIDABLE) {
          if (pointInAABB(bx, by, bz, aabb)) {
            bullet.alive = false;
            stopped = true;
            break;
          }
        ...
        // ── Player hit (bot bullets only) ─────────────────────────
        if (!stopped && bullet.ownerType === 'bot' && sharedState.playerAlive) {
    ...
    for (let i = 0; i < bulletMeshes.current.length; i++) {
      ...
      if (bullet) {
        ...
        slot.mesh.visible = slot.trail.visible = false;
      }
    }
  });
